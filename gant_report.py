...
project = pd.DataFrame(columns=['issue', 'start', 'action_by', 'from', 'status'])
...
for issue in jira.search_issues(jql, maxResults=500):
    ...
    if ((str(issue.fields.status) != 'Done') and (str(issue.fields.status) != 'Invalid')):            
        action = {'issue': str(issue.key), 'start': datetime.now(), 'action_by': issue.fields.reporter.displayName, 'from': "", 'status': str(issue.fields.status)}
        project = project.append(action, ignore_index=True)
...
fig = px.timeline(project, x_start='start', x_end='end', y='issue', color='status' ,title=(epic_name + ' - Total Number of Tickets:' + str(len(project['issue'].unique()))))
