...
def create_gantt(issues, title="Gantt Chart", start_date=datetime.min):
    ...
    for issue in issues:
        ...
        start_time = datetime.strptime(
            str(issue.fields.created)[:19], "%Y-%m-%dT%H:%M:%S"
        )
    ...
    df = df.sort_values(by=["issue", "start"], ascending=[False, True]).reset_index(
        drop=True
    )
    ...
    fig = px.timeline(
        ...
        y="issue",
        color="status",
        title=(title + " - Total Number of Tickets:" + str(len(df["issue"].unique()))),
    )
